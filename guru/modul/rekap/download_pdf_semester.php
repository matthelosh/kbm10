...
// ✅ Ambil info mengajar (guru, mapel, kelas)
...
// ✅ Ambil semester & tahun ajaran dari semester_id user
...
$d['semester'] = $infoSemester['semester'];
$d['tahun_ajaran'] = $infoSemester['tahun_ajaran'];
...
// ✅ Hitung periode tanggal semester
$tahun = explode('/', $d['tahun_ajaran']);
$tahunAwal = intval($tahun[0] ?? date('Y'));
$tahunAkhir = intval($tahun[1] ?? (date('Y') + 1));

$startDate = strtolower($d['semester']) === 'ganjil'
    ? "$tahunAwal-07-01"
    : "$tahunAkhir-01-01";
$endDate = strtolower($d['semester']) === 'ganjil'
    ? "$tahunAwal-12-31"
    : "$tahunAkhir-06-30";
...
<h4 style="text-align:center;">REKAP ABSENSI SEMESTER '.strtoupper($d['semester']).' ('.$d['tahun_ajaran'].')</h4>
...
$no = 1;
$siswa = mysqli_query($con, "SELECT id_siswa, nis, nama_siswa, jk FROM tb_siswa WHERE id_mkelas='$id_mkelas' ORDER BY nama_siswa ASC");
...
$dompdf->render();
$filename = "Rekap_Absensi_Semester_{$d['nama_kelas']}_".strtoupper($d['semester']).".pdf";
