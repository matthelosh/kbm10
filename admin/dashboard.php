...
// Handler bulk delete
if (isset($_GET['act']) && $_GET['act'] == 'bulk_delete') {
    
    if (isset($_POST['selected_siswa']) && is_array($_POST['selected_siswa']) && count($_POST['selected_siswa']) > 0) {
        $ids = array_map('intval', $_POST['selected_siswa']);
        $id_list = implode(',', $ids);

        $delete = mysqli_query($con, "DELETE FROM tb_siswa WHERE id_siswa IN ($id_list)");

        if ($delete) {
            $_SESSION['success_message'] = "Berhasil menghapus " . mysqli_affected_rows($con) . " data siswa!";
        } else {
...
if (isset($_GET['act']) && $_GET['act'] == 'bulk_delete_jurnal') {
    if (isset($_POST['selected_jurnal']) && is_array($_POST['selected_jurnal']) && count($_POST['selected_jurnal']) >0) {
        $ids = array_map('intval', $_POST['selected_jurnal']);
        $id_list = implode(',',$ids);
        
        $delete = mysqli_query($con, "DELETE FROM jurnal_mengajar WHERE id IN ($id_list)");
        if ($delete) {
            $_SESSION['success_message'] = 'Berhasil menghapus' . mysqli_affected_rows($con) . ' data jurnal';
        } else {
            $_SESSION['error_message'] = 'Gagal menghapus data!' . mysqli_error($con);
        }
    } else {
        $_SESSION['error_message'] = 'Tidak ada data jurnal untuk dihapus';
    }
    
    header("Location: dashboard.php?page=jurnal");
    exit;
    
}

if (isset($_GET['act']) && $_GET['act'] == 'delete_lag') {
    if (isset($_POST['selectedAbsen']) && is_array($_POST['selectedAbsen']) && count($_POST['selectedAbsen']) > 0) {
        $ids = array_map('intval', $_POST['selectedAbsen']);
        $list = implode(',',$ids);
        
        $delete = mysqli_query($con, "DELETE FROM _logabsenguru WHERE id_absguru IN ($list)");
        if ($delete) {
            $_SESSION['success_message'] = 'Absensi guru berhasil dihapus: '. mysqli_affected_rows($con) .' data';
        } else {
            $_SESSION['error_message'] = 'Gagal menghapus absensi guru.' . mysqli_error($con);
        }
    } else {
        $_SESSION['error_message'] = 'Tidak ada absen yang terpilih.';
    }
    
    header("Location: dashboard.php?page=guru&act=rekap");
    exit;
}
		...
		<div class="main-header">
			...
			<div class="logo-header" data-background-color="blue">
				...
				<a href="dashboard.php" class="logo">
					<!-- <img src="/assets/img/vsc.png" alt="navbar brand" class="navbar-brand" width="40"> -->
						...
						<li class="nav-item">
							...
							<div class="collapse" id="base">
								<ul class="nav nav-collapse">
									...
									<li class="<?= ($_GET['page'] == 'jurnal') ? 'active' : '' ?>">
  										<a href="?page=jurnal">
			...
			<div class="content">
				...
				if ($page == 'master') {
					...
					// Jurnal
						...
						include 'master/jurnal/jurnal.php';
					}
				...
				// Halaman JURNAL
					...
					if ($act == '') {
						include 'modul/master/jurnal/data.php';
					} elseif ($act == 'add') {
						include 'modul/jadwal/tambah.php';
					} elseif ($act == 'cancel') {
						include 'modul/jadwal/cancel.php';
				...
				// Halaman REKAP JURNAL
				} elseif ($page == 'rekap_jurnal') {
					include 'admin/modul/master/jurnal/data.php';
